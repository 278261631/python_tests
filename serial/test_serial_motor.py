# pip install pyserial

import serial
import schedule
import time


crc_hi_table = [
    
]

crc_lo_table = [
    
]


def crc16(data):
    crc_hi = 0xFF
    crc_lo = 0xFF
    for byte in data:
        index = crc_hi ^ byte
        crc_hi = crc_lo ^ crc_hi_table[index]
        crc_lo = crc_lo_table[index]
    return (crc_hi << 8) | crc_lo


ser = serial.Serial(
    port='COM9',
    baudrate=19200,
    bytesize=8,
    parity='N',
    stopbits=1,
    timeout=2
)

# 检查串口是否打开
if ser.isOpen():
    print("串口已打开")


def go_next():
    try:
        data = bytes([0x01, 0x10, 0x00, 0x0C, 0x00, 0x02, 0x04, 0xFF, 0xFF, 0x00, 0x00])
        # data = bytes([0x02, 0x10, 0x00, 0x0C, 0x00, 0x02, 0x04, 0xFF, 0xFF, 0x00, 0x00])
        crc_result = crc16(data)
        crc_hi = (crc_result >> 8) & 0xFF
        crc_lo = crc_result & 0xFF
        extended_data = data + bytes([crc_hi, crc_lo])
        formatted_data = ' '.join(f'{byte:02x}' for byte in extended_data)
        # print(f"-->: {extended_data.hex()}    {formatted_data }")
        ser.write(extended_data)
    except Exception as ex:
        print("发生错误:", ex)
    time.sleep(0.5)


def go_back():
    try:
        data = bytes([0x01, 0x7B, 0xFF, 0xFF, 0xFF, 0xFF])  # save
        # data = bytes([0x02, 0x7B, 0xFF, 0xFF, 0xFF, 0xFF])  # save
        crc_result = crc16(data)
        crc_hi = (crc_result >> 8) & 0xFF
        crc_lo = crc_result & 0xFF
        extended_data = data + bytes([crc_hi, crc_lo])
        formatted_data = ' '.join(f'{byte:02x}' for byte in extended_data)
        # print(f"-->: {extended_data.hex()}    {formatted_data }")
        ser.write(extended_data)
    except Exception as ex:
        print("发生错误:", ex)


test_counter = 0


def job():
    global test_counter
    test_counter = test_counter + 1
    if test_counter % 10 == 1:
        print(test_counter)
    go_next()
    go_next()
    go_next()
    go_back()


schedule.every(1).seconds.do(job)


while True:
    schedule.run_pending()
    time.sleep(1)

