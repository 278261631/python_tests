# 定义CRC高位字节值表
crc_hi_table = [
    
]

# 定义CRC低位字节值表
crc_lo_table = [
    
]


def crc16(data):
    crc_hi = 0xFF
    crc_lo = 0xFF
    for byte in data:
        index = crc_hi ^ byte
        crc_hi = crc_lo ^ crc_hi_table[index]
        crc_lo = crc_lo_table[index]
    return (crc_hi << 8) | crc_lo


# 示例使用
if __name__ == "__main__":
    # data = bytes([0x01, 0x06, 0x00, 0x00, 0x00, 0x01])
    # data = bytes([0x01, 0x7a, 0x00, 0x15, 0x00, 0x02])  write 01 id to 02
    # data = bytes([0x01, 0x03, 0x00, 0x15, 0x00, 0x01])  read 01 id
    # data = bytes([0x01, 0x03, 0x00, 0x15, 0x00, 0x01])  #read 01 id
    # data = bytes([0x01, 0x03, 0x00, 0x02, 0x00, 0x01])  #read 01 speed
    # data = bytes([0x02, 0x03, 0x00, 0x02, 0x00, 0x01])  #read 01 speed
    # data = bytes([0x02, 0x03, 0x00, 0x15, 0x00, 0x01]) read 02  id
    # data = bytes([0x01, 0x03, 0x00, 0x03, 0x00, 0x01])   # read 01 accel
    # data = bytes([0x02, 0x03, 0x00, 0x03, 0x00, 0x01])   # read 02 accel
    # data = bytes([0x01, 0x06, 0x00, 0x02, 0x09, 0xC4])    #write 01 speed = 2500
    # data = bytes([0x01, 0x06, 0x00, 0x03, 0xc3, 0x50])    # write 01 accel = 50000
    # data = bytes([0x02, 0x06, 0x00, 0x03, 0xc3, 0x50])    # write 02 accel = 50000
    # data = bytes([0x02, 0x10, 0x00, 0x0C, 0x00, 0x02, 0x04, 0x0f, 0xA0, 0x00, 0x00])  # go 02 +4000
    # data = bytes([0x02, 0x10, 0x00, 0x0C, 0x00, 0x02, 0x04, 0x80, 0x00, 0x00, 0x00])  # go 02 +32768
    # data = bytes([0x02, 0x10, 0x00, 0x0C, 0x00, 0x02, 0x04, 0x00, 0x00, 0x00, 0x05])  # go 02 +327680
    # data = bytes([0x02, 0x10, 0x00, 0x0C, 0x00, 0x02, 0x04, 0x00, 0x00, 0x00, 0x32])  # go 02 +327680

    data = bytes([0x02, 0x06, 0x00, 0x14, 0x00, 0x01])  # save
    # 计算CRC校验结果
    crc_result = crc16(data)
    # 将CRC结果转换为两个单字节，分别表示高字节和低字节
    crc_hi = (crc_result >> 8) & 0xFF
    crc_lo = crc_result & 0xFF
    # 将CRC结果追加到数据末尾
    extended_data = data + bytes([crc_hi, crc_lo])
    # 打印原始数据和CRC结果
    print(f"原始数据: {data.hex()}")
    print(f"CRC校验结果: {crc_hi:02X} {crc_lo:02X}")
    # 打印包含CRC的扩展数据
    print(f"包含CRC的数据: {extended_data.hex()}")
    formatted_data = ' '.join(f'{byte:02x}' for byte in extended_data)
    print(f"包含CRC的数据: {formatted_data}")
    print(f"CRC校验结果: {crc_result:04X}")

